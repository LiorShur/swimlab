...
- `io.py` is **drafted against the DOT format, cross-checked against a reference BLE parser** (`jiminghe/Xsens_DOT_PC_Reader`) + Movella's documented CSV columns, but **not yet run on a real export**. Field layout/types/units and the free-vs-raw acceleration split are pinned; the rest stays `# TODO(real-file)`. When the first real recording exists, run `io.validate_dot_export(path)` — PASS/WARN/FAIL per assumption (columns, sample rate, quaternion norm, acceleration-with-gravity, ENU-vs-NED gravity frame) — then correct anything flagged. **Recording mode:** use **Custom Mode 5** (Quaternion + raw Acceleration + Angular velocity) at 120 Hz — it yields the canonical schema minus the unused magnetometer, with gravity in `Acc_*` (no reconstruction). Orientation modes (Complete/Extended Quaternion, etc.) instead log **`FreeAcc_*`** (gravity removed); `io.py` reads those too and reconstructs gravity from the quaternion. Note 120 Hz is *recording-only* (BLE streaming caps at 60 Hz). Until a real file exists, `read_dot_export` is exercised against synthetic CSVs in the real column format (self-consistency, not format confirmation).
...
- **Water affects raw acc/gyro *magnitudes* (Padrela 2024, MSc, Xsens waterproof IMU).** Their proposed additive "correction factor" (force in-water peaks to match out-of-water) is **not** for us — it corrects the wrong direction for a swimming-native tool and conflates drag-altered kinematics with sensor artifact; and our primary metrics are **T0-calibrated gravity-referenced angles** from the fused quaternion, which are insulated. Confirmatory only (60 Hz stream cap → record onboard at 120 Hz; acc/gyro not mag/yaw; sensor enclosure/placement doesn't matter → lean on T0). **Actionable:** the raw-*magnitude* features — `config.pushoff_g_threshold` (`events.py`) and the placement detector's gyro-RMS/p95 — must be calibrated/validated on real **in-water** data, not dry-land. Full write-up in the product repo's `docs/related-work.md`.
